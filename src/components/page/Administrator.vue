<template>
	<div>
		...
		<div class="transferForm">
			<el-transfer v-model="users" ref="transferRef" filterable filter-placeholder="请输入用户的昵称或Email" :data="usersData" :titles="['恢复用户列表', '禁用用户列表']" :button-texts="['恢复', '禁用']" :footer-format="{
        			noChecked: '${total}个用户',
        			hasChecked: '已选${checked} / ${total}个用户'
        		}" @change="handleChange">
				<el-button class="transfer-footer" slot="left-footer" :plain="true" type="danger" icon="delete" size="mini" @click="deleteLeftUsers">
					删除
				</el-button>
				<el-button class="transfer-footer" slot="right-footer" :plain="true" type="danger" icon="delete" size="mini" @click="deleteRightUsers">
					删除
				</el-button>
			</el-transfer>
		</div>
